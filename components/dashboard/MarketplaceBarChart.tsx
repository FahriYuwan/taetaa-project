...
interface MarketplaceChartData {
  name: string;
  revenue: number;
}

interface MarketplaceBarChartProps {
  data: MarketplaceChartData[];
}

export function MarketplaceBarChart({ data }: MarketplaceBarChartProps) {
  return (
    <div
      style={{
      ...
      {data.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data}>
            ...
            />
            <Bar dataKey="revenue" fill={colors.brand[500]} radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
